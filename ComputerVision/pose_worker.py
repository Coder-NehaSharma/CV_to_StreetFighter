...
class PoseWorker:
    ...
    def _run(self):
        try:
                ...
                while not self._stop_event.is_set():
                    ...
                    if results.pose_landmarks:
                        ...
                        if visible_x_coords:
                            ...
                            # --- Zone-Based Movement (Virtual Joystick) ---
                            # This is more intuitive: Position in camera frame maps to movement.
                            # Left Zone | Dead Zone (Center) | Right Zone
                            
                            left_threshold = 0.35  # If body is in left 35% of screen
                            right_threshold = 0.65 # If body is in right 35% of screen
                            ...
                            movement_info["x_diff"] = 0.0 # Deprecated but kept for structure
                    ...
                    # Punch Detection: Wrist close to shoulder height and extended
                    # Correct Mapped Indices for filtered vector (size 21):
                    # 11(L.Shoulder)->7, 12(R.Shoulder)->8
                    # 15(L.Wrist)->11, 16(R.Wrist)->12
                    # 23(L.Hip)->13
                    # 27(L.Ankle)->17, 28(R.Ankle)->18
                    
                    # Check for Punch (Left or Right)
                    left_wrist_y = vector[11*4+1]     # Original 15 -> 11
                    left_shoulder_y = vector[7*4+1]   # Original 11 -> 7
                    right_wrist_y = vector[12*4+1]    # Original 16 -> 12
                    right_shoulder_y = vector[8*4+1]  # Original 12 -> 8
                    
                    # Simple Punch Check: Wrist roughly at shoulder height (+/- threshold)
                    punch_threshold = 0.2
                    if (abs(left_wrist_y - left_shoulder_y) < punch_threshold) or (abs(right_wrist_y - right_shoulder_y) < punch_threshold):
                         attacks[3] = 1

                    # Check for Kick (Left or Right)
                    left_ankle_y = vector[17*4+1]     # Original 27 -> 17
                    right_ankle_y = vector[18*4+1]    # Original 28 -> 18
                    # left_hip_y = vector[13*4+1]     # Original 23 -> 13
                    
                    # Simple Kick Check: Ankle raised relative to other ankle
                    kick_threshold = 0.4 
                    
                    if abs(left_ankle_y - right_ankle_y) > 0.15: # One leg is lifted
                         attacks[0] = 1

                    # --- Prediction into Game inputs
                    if pred_label == "punch" or attacks[3] == 1:
                        attacks[3] = 1
                    elif pred_label == "kick" or attacks[0] == 1:
                        attacks[0] = 1
            ...
            print(f"PoseWorker thread crashed: {e}")
            import traceback
            traceback.print_exc()
            if 'cap' in locals():
                cap.release()
